...
,comment		            nvarchar2(200)                           -- 댓글내용(시)
,registerdate		        DATE DEFAULT SYSDATE NOT NULL            -- 댓글작성일자
,fk_userid		            nvarchar2(20)                            -- 아이디(FK)

,constraint PK_tbl_clubboardcomment_clcmtseq primary key(clubboardcommentseq)
,constraint FK_tbl_clubboardcomment_clubboardseq foreign key(clubboardseq) references tbl_clubboard(clubboardseq)
,constraint FK_tbl_clubboardcomment_fk_userid foreign key(fk_userid) references tbl_member(userid)
);
