...
public class GoogleMail {
	...
	public void send_certification_code(String recipient, String certification_code) throws Exception { 
	      ...
	      // 1. 정보를 담기 위한 객체
	       ...
	       prop.put("mail.smtp.user", "leenaraeleenarae@gmail.com"); 
	    ...
	    */ 
	       ...
	       // 제목 설정
	       String subject = "아마두 ID 찾기 인증번호 발송";
	...
	public void sendmail_OrderFinish(String recipient, String name, String emailContent) throws Exception {
	    ...
	    */ 
	       ...
	       // 메시지 본문의 내용과 형식, 캐릭터 셋 설정
	       msg.setContent("<div style='font-size:14pt; color:red;'>"+emailContent+"</div>", "text/html;charset=UTF-8");
	...
	public void sendmail_withFile(Map<String, Object> paraMap) throws Exception {
		...
		String senderEmail = paraMap.get("userid")+"@amado.com";
	       ...
	       if(paraMap.get("arr_attachFilename") == null) { // 첨부파일이 없으면 내용만 전송한다.
	          ...
	          if(paraMap.get("arr_attachFilename") != null) {
	             ...
	             for(String attachFilename : arr_attachFilename) {
	                ...
	                // 다음은 탐색기에서 C:\NCS\workspace_spring_framework\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\board\resources\email_attach_file에 생성된 파일을 불러온다. 
	   ...
	   public void sendmail_iqraws(Map<String, String> paraMap) throws Exception {
	    ...
	    */ 
	       ...
	       // 제목 설정
	       String subject = "amado 문의 답변이 도착했습니다.";
	       msg.setSubject(subject);
	               
	       // 보내는 사람의 메일주소
	       String sender = "rtw1024sj@gmail.com";
	       Address fromAddr = new InternetAddress(sender);
	       msg.setFrom(fromAddr);
	       ...
	       // 메시지 본문의 내용과 형식, 캐릭터 셋 설정
	       msg.setContent(paraMap.get("content"), "text/html;charset=UTF-8");
